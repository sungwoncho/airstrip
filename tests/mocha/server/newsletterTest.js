...
describe("campaignBuilder.run", function(){
  beforeEach(function () {
    Flights.remove({});
    Items.remove({});
  });
  ...
  it("builds and returns campaign with correct attributes", function(){
    ...
    expect(result.text).to.include('example1');
    expect(result.text).to.include('example2');
    expect(result.text).to.include('example3');
